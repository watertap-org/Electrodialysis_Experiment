...
#################################################################################
# No code needed here; this is just the copyright header.
...
@declare_process_block_class("ED_base")
class ED1DData(InitializationMixin, UnitModelBlockData):
    """
    1D Electrodialysis Base Model
    """
    ...
    **Valid values:** {
    ...
    def build(self):
        ...
        self.spacer_porosity = Var(
            ...
            bounds=(0.01, 1),
            units=pyunits.dimensionless,
            doc='The prosity of spacer in the ED channels. This is also referred to elsewhere as "void fraction"',
        )
        ...
        self.membrane_areal_resistance_x = Var(
            self.membrane_set,
            self.diluate.length_domain,
            initialize=2e-4,
            bounds=(1e-6, 1),
        ...
        def current_efficiency_rule(b, t, x):
            return (
                -sum(
                    self.diluate.mass_transfer_term[t, x, "Liq", j]
                    * self.config.property_package.charge_comp[j]
                    for j in self.cation_set
                )
                * Constants.faraday_constant
                * (self.current_density_x[t, x] * self.cell_width * self.cell_pair_num)
                ** -1
            )

        self.current_efficiency_x = Expression(
            self.flowsheet().time,
            self.diluate.length_domain,
            rule=current_efficiency_rule,
        )

        def velocity_diluate_exp_rule(b, t, x):
            return self.diluate.properties[t, x].flow_vol_phase["Liq"] / (
                self.cell_width
                * self.channel_height
        ...
        self.velocity_diluate = Expression(
            self.flowsheet().time,
            self.diluate.length_domain,
            rule=velocity_diluate_exp_rule,
        )

        def velocity_concentrate_exp_rule(b, t, x):
            return self.concentrate.properties[t, x].flow_vol_phase["Liq"] / (
                self.cell_width
                * self.channel_height
                * self.spacer_porosity
                * self.cell_pair_num
        ...
        self.velocity_concentrate = Expression(
            self.flowsheet().time,
            self.diluate.length_domain,
            rule=velocity_concentrate_exp_rule,
        )
    ...
    def _make_performance(self):
        ...
        if (
            ...
            def eq_deltaP_diluate(self, t, x):
                return self.diluate.deltaP[t, x] + self.pressure_drop[t] == 0
            ...
            def eq_deltaP_concentrate(self, t, x):
                return self.concentrate.deltaP[t, x] + self.pressure_drop[t] == 0
        ...
        @self.Constraint(
            ...
            self.membrane_set,
            self.diluate.length_domain,
            doc="Calculate membrane areal resistance at L=x",
        )
        def eq_get_membrane_areal_resisance_x(self, t, memb, x):
            return (
                self.membrane_areal_resistance_x[memb, x]
                * sum(
                ...
                * sum(
                ...
                + self.membrane_areal_resistance_coef[memb]
            )
        ...
        def eq_get_total_areal_resistance_x(self, t, x):
            if self.config.has_Nernst_diffusion_layer:
                return (
                    ...
                    - (
                        (
                            self.membrane_areal_resistance_x["aem", x]
                            + self.membrane_areal_resistance_x["cem", x]
                            + (
                                self.channel_height
                                - self.dl_thickness_x["cem", "cathode_left", t, x]
                                - self.dl_thickness_x["aem", "anode_right", t, x]
                            )
                            * self.spacer_conductivity_coefficient**-1
                            * self.concentrate.properties[t, x].elec_resistance_phase[
                                "Liq"
                            ]
                            + (
                                self.channel_height
                            ...
                            * self.spacer_conductivity_coefficient**-1
                            * self.diluate.properties[t, x].elec_resistance_phase["Liq"]
                        )
                        * self.cell_pair_num
                        + self.electrodes_resistance
                        + self.slack_resistance * self.cell_width * self.cell_length
                    )
                    == 0
                )
            else:
                        ...
                        (
                            ...
                            * (
                                ...
                                * self.concentrate.properties[
                                    t, x
                                ].elec_resistance_phase["Liq"]
                                + self.spacer_conductivity_coefficient**-1
                                * self.diluate.properties[t, x].elec_resistance_phase[
                                    "Liq"
                                ]
                            )
                        )
                ...
                if self.config.has_Nernst_diffusion_layer:
                    return (
                        ...
                        - (
                            -self.cell_width
                            * (
                                self.ion_trans_number_membrane["cem", j, x]
                                - self.ion_trans_number_membrane["aem", j, x]
                            )
    ...
    def _make_performance_nonohm_mem(self):
        ...
        def eq_potential_nonohm_membrane_x(self, mem, t, x):
            ...
            return (
                self.potential_nonohm_membrane_x[mem, t, x]
                - (
                    ...
                    * (
                        sum(
                            self.ion_trans_number_membrane[mem, j, x]
                            / self.config.property_package.charge_comp[j]
                            for j in self.cation_set
                        )
                        + sum(
    ...
    def _make_performance_dl_polarization(self):
        ...
        def eq_potential_nonohm_dl(self, mem, t, x):
            if mem == "cem":
                return (
                    self.potential_nonohm_dl_x[mem, t, x]
                    - (
                        Constants.gas_constant
                        * self.diluate.properties[t, x].temperature
                        / Constants.faraday_constant
        ...
        def eq_potential_ohm_dl_x(self, mem, t, x):
            if mem == "cem":
                return (
                    self.potential_ohm_dl_x[mem, t, x]
                    - (
                        Constants.faraday_constant
                        * (
                            sum(
        ...
        def dl_thickness_expr_rule(b, mem, side, t, x):
        ...
        self.dl_thickness_x = Expression(
            self.membrane_set,
            self.electrode_side_set,
            self.flowsheet().time,
            self.diluate.length_domain,
            rule=dl_thickness_expr_rule,
            doc="Expression for the thickness of the diffusion layer",
        )
    ...
    def _get_fluid_dimensionless_quantities(self):
            ...
            def eq_hydraulic_diameter(self):
                if (
                    ...
                    return (
                        ...
                        * (self.channel_height + self.cell_width)
                        - 2
                        * self.channel_height
                        * self.cell_width
                        * self.spacer_porosity
        ...
        def Re_expr_rule(b):
            return (
                b.dens_mass
                * b.velocity_diluate[0, 0]
                * b.hydraulic_diameter
                * b.visc_d**-1
            )

        self.N_Re = Expression(rule=Re_expr_rule)

        def Sc_expr_rule(b):
            return b.visc_d * b.dens_mass**-1 * b.diffus_mass**-1

        self.N_Sc = Expression(rule=Sc_expr_rule)

        def Sh_expr_rule(b):
            return 0.29 * b.N_Re**0.5 * b.N_Sc**0.33

        self.N_Sh = Expression(rule=Sh_expr_rule)
    ...
    def _pressure_drop_calculation(self):
        ...
        if self.config.pressure_drop_method == PressureDropMethod.experimental:
            if not self.pressure_drop[0].is_fixed:
                raise ConfigurationError(
                    "The pressure_drop must be fixed if "
                    "PressureDropMethod.experimental is selected."
                )
            ...
            def eq_pressure_drop(self, t):
                return (
                    self.pressure_drop[t] * self.hydraulic_diameter
                    - self.dens_mass
            ...
            if self.config.friction_factor_method == FrictionFactorMethod.fixed:
                ...
                def eq_friction_factor(self):
                    if (
                        ...
                        return (
                            self.friction_factor * self.N_Re
                            - 4 * 50.6 * self.spacer_porosity**-7.06
                            == 0
                        )
                    elif (
                        self.config.friction_factor_method
            ...
            self[k].total_areal_resistance_x[...].set_value(
                (
                    ...
                    * (
                        ...
                        + self[k].diluate.properties[set].elec_cond_phase["Liq"] ** -1
                    )
                )
    ...
    def calculate_scaling_factors(self):
        ...
        if iscale.get_scaling_factor(self.cell_pair_num, warning=False) is None:
            iscale.set_scaling_factor(self.cell_pair_num, 0.1)
        if iscale.get_scaling_factor(self.membrane_thickness, warning=False) is None:
            iscale.set_scaling_factor(self.membrane_thickness, 1e4)

        if (
            iscale.get_scaling_factor(
                self.membrane_areal_resistance_const, warning=False
            )
            is None
        ):
        ...
        if (
            iscale.get_scaling_factor(self.solute_diffusivity_membrane, warning=False)
            is None
        ):
            iscale.set_scaling_factor(self.solute_diffusivity_membrane, 1e10)

        if hasattr(self, "diffus_mass") and (
            iscale.get_scaling_factor(self.diffus_mass, warning=False) is None
        ):
            iscale.set_scaling_factor(self.diffus_mass, 1e9)

        if (
            iscale.get_scaling_factor(self.water_permeability_membrane, warning=False)
            is None
        ):
            iscale.set_scaling_factor(self.water_permeability_membrane, 1e14)

        if iscale.get_scaling_factor(self.channel_height, warning=False) is None:
