...
def initialize_dof0_system(
    ...
    if solve_after_init:
        ...
        res = solve(m, tee=tee, check_termination=terminate_nonoptimal_sol)
...
def solve(blk, solver=None, tee=True, linear_solver="ma27", check_termination=False):
    if solver is None:
        solver = get_solver()
...
def initialize_system(m, solver=None, report_bad_scaling=False):
    ...
    m.fs.properties.set_default_scaling("flow_vol_phase", 1e5, index=("Liq"))
    # m.fs.properties.set_default_scaling(
    ...
    sf_flow_mol_na_main_prop = iscale.get_scaling_factor(
        m.fs.properties.flow_mol_phase_comp["Liq", "Na_+"]
    )
    sf_flow_mol_mg_main_prop = iscale.get_scaling_factor(
        m.fs.properties.flow_mol_phase_comp["Liq", "Mg_2+"]
    )
    sf_flow_mol_ca_main_prop = iscale.get_scaling_factor(
        m.fs.properties.flow_mol_phase_comp["Liq", "Ca_2+"]
    )
    sf_flow_mol_cl_main_prop = iscale.get_scaling_factor(
        m.fs.properties.flow_mol_phase_comp["Liq", "Cl_-"]
    )

    iscale.set_scaling_factor(
        m.fs.mix0.from_conc_out_state[0].flow_mol_phase_comp["Liq", "Na_+"],
        sf_flow_mol_na_main_prop * init_sepa1_frac**-1,
    )
    iscale.set_scaling_factor(
        m.fs.mix0.from_conc_out_state[0].flow_mol_phase_comp["Liq", "Mg_2+"],
        sf_flow_mol_mg_main_prop * init_sepa1_frac**-1,
    )
    iscale.set_scaling_factor(
        m.fs.mix0.from_conc_out_state[0].flow_mol_phase_comp["Liq", "Ca_2+"],
        sf_flow_mol_ca_main_prop * init_sepa1_frac**-1,
    )
    iscale.set_scaling_factor(
        m.fs.mix0.from_conc_out_state[0].flow_mol_phase_comp["Liq", "Cl_-"],
        sf_flow_mol_cl_main_prop * init_sepa1_frac**-1,
    )
